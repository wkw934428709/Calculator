...
public class WheelScroller {
    ...
    public static final int MIN_DELTA_FOR_SCROLLING = 2;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (Math.abs(distanceY) >MIN_DELTA_FOR_SCROLLING) {
                startScrolling();
                listener.onScroll(distanceY);
                lastTouchedY = event.getY();
            }else{
                if( (int)Math.abs((lastClickY-event.getY())) <=MIN_DELTA_FOR_SCROLLING ){
                    lastClickY = event.getY();
                    listener.onClick(event.getY());
                }
            }
            break;
        }
